...
public class JwtUtil {
	...
	public static Optional<String> getUsername(String token) {
		...
		try {
			...
			Claims claims = Jwts.parserBuilder().setSigningKey(JWT_KEY.getBytes(StandardCharsets.UTF_8)) 
					// On redonne la signature pour validation
			...
			// On retourne le subject (le nom d'utilisateur) contenu dans le jeton
		}
